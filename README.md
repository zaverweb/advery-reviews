...
- **Purpose-built storage.** Reviews live in two lean, indexed tables (`wp_zaverweb_reviews` + a per-object aggregate cache `wp_zaverweb_review_stats`), not a comment row fanned out across many `commentmeta` rows. Rating summaries and schema output are a single indexed read.
...
- **Fully configurable.** Who can submit (anyone / logged-in), moderation (hold / auto-approve), one-per-user, rating-required, minimum length, per-page, auto-append vs `[zaverweb_reviews]` shortcode.
