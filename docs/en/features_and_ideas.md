...
- **G — Elementor Loop-Grid. — DROPPED (2026-09-03, user decision).** Would require a hidden `advery_review` CPT mirror (the only way Elementor Loop Grid + Dynamic Tags work, since both operate on WP_Query / the current post). That reintroduces exactly the post/meta fan-out the plugin was built to avoid, adds sync + resync + drift-risk maintenance, and is Elementor-Pro-only — for modest benefit given we already ship a native Elementor widget, a shortcode, a Gutenberg block, card skins and appearance controls. Not worth it now; revisit only if a real need appears.
