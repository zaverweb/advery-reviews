...
- **AntiSpam\\**: بررسی‌های لایه‌ای و امتیازمحور (هانی‌پات، زمان‌سنجی، قواعد لینک/کلمه، بلاک‌لیست، محدودیت نرخ، ایمیلِ یکبارمصرف، ارائه‌دهنده‌های اختیاریِ CAPTCHA، Akismet اختیاری). یک `SpamGuard` که امتیاز + دلیل برمی‌گرداند؛ آستانه‌ها به auto-approve / hold / spam نگاشت می‌شوند.
