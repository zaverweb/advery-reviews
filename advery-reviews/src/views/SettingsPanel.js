			...
			case 'antispam':
				return (
					<>
						...
						<div className="advery-rv-fieldgroup-title">{ __( 'Native WordPress comments', 'advery-reviews' ) }</div>
						<SelectControl label={ __( 'Guard the built-in WordPress comment form', 'advery-reviews' ) } value={ as.native_comment_guard || 'off' } options={ [ { label: __( 'Off — don’t touch native comments', 'advery-reviews' ), value: 'off' }, { label: __( 'Filter — apply the anti-spam rules to native comments', 'advery-reviews' ), value: 'filter' }, { label: __( 'Disable — refuse all native comments', 'advery-reviews' ), value: 'disable' } ] } onChange={ ( v ) => setAs( { native_comment_guard: v } ) } __nextHasNoMarginBottom />
						<p className="advery-rv-hint" style={ { marginTop: 0 } }>{ __( 'Spam bots POST straight to wp-comments-post.php, bypassing your review form — and “comments closed” is per-post, so any open post still accepts them. “Filter” runs the link, blocked-word and blocked/disposable-email rules above on native comments too (blocked → rejected, links/throwaway emails → spam or held). “Disable” refuses every native comment and hides the comment form site-wide. Logged-in users who can moderate comments are never blocked.', 'advery-reviews' ) }</p>
						<hr />
						...
						<p className="advery-rv-hint" style={ { marginTop: 0 } }>{ __( 'Detects real, marked-up and obfuscated links (http://…, www., <a>, [url], 1.2.3.4, “example dot com”, “example[.]com”) across review, title and name. “Reject” blocks them for everyone; “Spam”/“Hold” keep them hidden for you to review.', 'advery-reviews' ) }</p>
						<div className="advery-rv-fields advery-rv-fields--wide">
							<TextareaControl rows={ 4 } label={ __( 'Strict link endings / patterns (optional)', 'advery-reviews' ) } help={ __( 'Leave empty for the safe default. One per line. A domain ending — .com, ru, .co.uk — makes a plain “word.ending” (e.g. shop.com) count as a link too. Advanced: a line starting with re: is a regular expression, e.g. re:\\bt\\.me/. Use this only if bots are posting bare domains like “myshop.ru” without http:// — it can also flag look-alikes (a sentence with “file.com” in it).', 'advery-reviews' ) } value={ as.link_tlds || '' } onChange={ ( v ) => setAs( { link_tlds: v } ) } __nextHasNoMarginBottom /></div>
			...
			case 'appearance': {
				...
				return (
					<>
						...
						<div className="advery-rv-fields advery-rv-fields--wide">
							<SelectControl label={ __( 'Layout style', 'advery-reviews' ) } help={ __( 'How each review looks. Cards = bordered cards. Classic = a simple divided list. Minimal = airy and borderless. Bubble = chat-style speech bubbles. Quote = centered testimonials with a large quote mark.', 'advery-reviews' ) } value={ ap.skin || 'card' } options={ [ { label: __( 'Cards (modern)', 'advery-reviews' ), value: 'card' }, { label: __( 'Classic list', 'advery-reviews' ), value: 'classic' }, { label: __( 'Minimal', 'advery-reviews' ), value: 'minimal' }, { label: __( 'Bubble (chat)', 'advery-reviews' ), value: 'bubble' }, { label: __( 'Quote (testimonial)', 'advery-reviews' ), value: 'quote' } ] } onChange={ ( v ) => setAp( { skin: v } ) } __nextHasNoMarginBottom />
